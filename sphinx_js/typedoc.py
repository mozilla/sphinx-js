...
class Typedoc:
    ...
    def get_parent(self, node):
        ...
        return self.nodelist[parentId] if parentId!=None else None

    def make_longname(self, node):
        parent = self.get_parent(node)
        longname = self.make_longname(parent) if parent!=None else ""
    ...
    def make_result(self, param):
        type = param.get('type')
        if type==None or type.get('name')=='void':
            return []
        return [self.make_doclet(
            name = param.get('name'),
    ...
    def convert_node(self, node):
        ...
        if kindString == 'External module':
            ...
            if node.get('flags',{}).get('isExported'):
                specifiers.append('*exported from* :js:mod:`'+self.get_parent(node).get('name')[1:-1]+'`')
            ...
            if node.get('getSignature'):
                ...
                type = self.make_type(node['setSignature']['parameters'][0]['type'])
